...
async function main() {
  ...
  for (const user of users) {
    try {
      await setUser(user)
      console.log(
        `✅ Set user:${user.fid} - ${user.displayName} (@${user.username})`
      )
    } catch (error) {
      console.error(`❌ Failed to set user:${user.fid}:`, error)
    }
  }

  console.log("User data setting completed!")
}
